...
// Bibliotecas para o Protocolo OLSR
...
main (int argc, char *argv[])
{
  ...
  CsmaHelper csma;
  csma.SetChannelAttribute ("DataRate", StringValue ("11Mbps"));
  csma.SetChannelAttribute ("Delay", TimeValue (NanoSeconds (656000))); // 0.000656 Segundos d
  ...
  AnimationInterface::SetConstantPosition (p2pNodes.Get (1), -2, -2); 
  AnimationInterface::SetConstantPosition (csmaNodes.Get (1), -2, -2); 

  // energy module 
  Ptr<BasicEnergySource> energySource = CreateObject<BasicEnergySource>();
  Ptr<SimpleDeviceEnergyModel> energyModel = CreateObject<SimpleDeviceEnergyModel>();

  energySource->SetInitialEnergy (800);
  energyModel->SetEnergySource (energySource);
  energySource->AppendDeviceEnergyModel (energyModel);
  energyModel->SetCurrentA (0.0022);

  // aggregate energy source to node
  // wifiApNode.Get (0)->AggregateObject (energySource);
  // wifiApNode.Get (1)->AggregateObject (energySource);
  csmaNodes.Get (0)->AggregateObject (energySource);
