...
void DelayMonitor(FlowMonitorHelper *fmHelper, Ptr<FlowMonitor> flowMon, Gnuplot3dDataset Dataset3);
...
//-----------------FlowMonitor-DELAY---------------------
  ...
  std::string dataTitle3       = "Delay";
      ...
      Gnuplot3dDataset dataset3;
      dataset3.SetTitle(dataTitle3);
      //dataset3.SetStyle(Gnuplot3dDataset::LINES_POINTS);
  ...
  void ThroughputMonitor (FlowMonitorHelper *fmhelper, Ptr<FlowMonitor> flowMon,Gnuplot2dDataset DataSet)
    {
      ...
      for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator stats = flowStats.begin (); stats != flowStats.end (); ++stats)
      {
        ...
        if((fiveTuple.sourceAddress == "10.1.1.13" && fiveTuple.destinationAddress == "10.1.1.21")){
...
void  DelayMonitor(FlowMonitorHelper *fmHelper, Ptr<FlowMonitor> flowMon, Gnuplot3dDataset Dataset3)
	{
		//double localDelay=0;
		double LastTxPacket=0;
		double LastRxPacket=0;
		...
		for(std::map<FlowId, FlowMonitor::FlowStats>::const_iterator stats3 = flowStats3.begin(); stats3 != flowStats3.end(); ++stats3)
		{
		        ...
		        if((fiveTuple3.sourceAddress == "10.1.1.13" && fiveTuple3.destinationAddress == "10.1.1.21")){
		        std::cout<<"Flow ID : "<< stats3->first <<";"<< fiveTuple3.sourceAddress <<"------>" <<fiveTuple3.destinationAddress<<std::endl;
		        LastTxPacket = stats3->second.timeFirstTxPacket.GetSeconds();
		        LastRxPacket = stats3->second.timeFirstRxPacket.GetSeconds();

		        //localDelay = stats3->second.timeLastTxPacket.GetSeconds()-stats3->second.timeLastRxPacket.GetSeconds();
		        Dataset3.Add((double)Simulator::Now().GetSeconds(), (double) LastTxPacket, (double) LastRxPacket);
